...
4.空間複雜度：O(n)，加法/減法生成新多項式，乘法可能最多 n×m 項

5.加分項：available-space list 可重用節點，降低記憶體分配開銷
...
| 測試案例 | 輸入 S | 預期輸出 | 實際輸出 |
|----------|-------------|----------|----------|
| 測試三   | p1: 3x² + 5, p2: 2x² + 1   | p1+p2=5x²+6, p1-p2=x²+4, p1*p2=6x⁴+13x²+5, p1(2)=17 | 正確       |
| 測試四   | p1: x³ + 2x, p2: x² + 1| p1+p2=x³+x²+2x+1, p1-p2=x³-x²+2x-1, p1*p2=x⁵+3x³+2x, p1(1)=3 | 正確      |
